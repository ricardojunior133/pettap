...
export default async function AccountPage() {
  ...
  return (
    <section aria-labelledby="account-heading">
      ...
      <section className="mt-9" aria-labelledby="pets-heading"><h2 id="pets-heading" className="text-xl font-semibold">Your pets</h2>{pets.length===0?<p className="mt-3 text-sm text-neutral-600">You have no pets set up yet.</p>:<ul className="mt-4 grid gap-3 sm:grid-cols-2">{pets.map((pet)=><li key={pet.publicIdentifier}><Card variant="outlined" className="p-5"><p className="font-semibold">{pet.name}</p><Link className="mt-3 inline-flex text-sm font-semibold underline underline-offset-4" href={`/account/pets/${encodeURIComponent(pet.publicIdentifier)}`}>Manage Lost Mode</Link></Card></li>)}</ul>}</section>
    </section>
  );
}
