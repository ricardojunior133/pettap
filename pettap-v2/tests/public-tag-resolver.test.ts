...
const base={tagId:"tag",accountId:"account",petId:"pet",pet:{id:"pet",accountId:"account",name:"Charlie",species:"Dog",breed:"Pug",birthDate:"2024-01-01",publicProfileEnabled:true,archivedAt:null},preferences:{showName:true,showBreed:true,showPhoto:false,showAge:true,publicMessage:"Please keep me safe."},photoPath:null,activationCount:1,lostReport:null};
...
describe("public NFC resolver",()=>{
 ...
 it("allowlists only consented profile data",async()=>expect(await service({...base,status:"active"}).resolveByPublicCode({publicCode:"tag-ok",ip:"test"})).toEqual({state:"active",profile:{name:"Charlie",species:"Dog",breed:"Pug",age:"2 years old",message:"Please keep me safe."}}));
 ...
 it("returns a strict Lost DTO only for a matching open report",async()=>expect(await service({...base,status:"lost",lostReport:{petId:"pet",tagId:"tag",openedAt:new Date("2026-07-01T12:00:00Z"),status:"open"}}).resolveByPublicCode({publicCode:"tag-ok",ip:"lost"})).toEqual({state:"lost",profile:{name:"Charlie",species:"Dog",breed:"Pug",age:"2 years old",message:"Please keep me safe.",reportedAt:"2026-07-01"}}));
 ...
 it("does not select or expose contacts, credentials, medical payloads or internal identifiers",()=>{const source=readFileSync("features/nfc/repositories/public-tag-repository.ts","utf8");expect(source).not.toMatch(/emergencyContacts|medicalInformation|nfcTagCredentials|credentialHash|phone|email/);});
});
