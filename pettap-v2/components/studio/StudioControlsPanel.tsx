...
function DesignStep({ studio, onUpdate }: { studio: StudioProps; onUpdate: UpdateStudio }) {
  ...
  return <><StepIntro number="2" title="Choose a design" description="Pick one of the five designs in your selected collection." />
    ...
    <fieldset><legend className="text-sm font-semibold text-neutral-900">Collection designs</legend><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">{models.map((item) => <OptionButton key={item.id} active={studio.design === item.id} onClick={() => onUpdate({ design: item.id })}><Image alt="" aria-hidden="true" className="mx-auto aspect-square w-20 object-contain" height={96} src={item.image} width={96} /><span className="mt-3 block text-sm font-semibold">{item.name}</span></OptionButton>)}</div></fieldset>
