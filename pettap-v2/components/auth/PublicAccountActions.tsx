...
export function PublicAccountActions({ isAuthenticated, compact = false }: { isAuthenticated: boolean; compact?: boolean }) {
  ...
  return <><Link href="/login" className={`inline-flex min-h-11 items-center rounded-xl border border-black/[0.09] bg-white px-4 text-sm font-semibold text-neutral-800 transition hover:bg-neutral-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-neutral-950/15 ${shared}`}>Log in</Link><Link href="/register" className={`inline-flex min-h-11 items-center rounded-xl bg-neutral-950 px-4 text-sm font-semibold text-white transition hover:bg-neutral-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-neutral-950/15 ${shared}`}>Create account</Link></>;
}
