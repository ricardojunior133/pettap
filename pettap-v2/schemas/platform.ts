...
export const publicProfileVisibilitySchema = z.object({ showBreed: z.boolean(), showPhoto: z.boolean(), showMedicalAlert: z.boolean(), contactActionsEnabled: z.boolean(), locationSharingEnabled: z.boolean() });
export const tagActivationSchema = z.object({ tagIdentifier: z.string().trim().toUpperCase().regex(/^[A-Z0-9]{6,32}$/, "Enter a valid PetTap identifier.") });
export const accountSettingsSchema = z.object({ locale: z.string().trim().min(2).max(16), theme: z.enum(["light", "dark", "system"]), marketingConsent: z.boolean() });
...
export type TagActivationInput = z.infer<typeof tagActivationSchema>;
